public class QueuePassenger04 {
    
    int max, size, front, rear;
    Passenger04[] Q;

    public QueuePassenger04(int n) {
        max = n;
        Create();
    }

    public void Create() {
        Q = new Passenger04[max];
        size = 0;
        front = rear = -1;
    }
    ...
    public void Enqueue(Passenger04 data) {
        if (IsFull()) {
    ...
    public Passenger04 Dequeue() {
        Passenger04 data = new Passenger04("", "", "", 0, 0);
        if (IsEmpty()) {
            System.out.println("Queue is still empty");
        } else {
            data = Q[front];
